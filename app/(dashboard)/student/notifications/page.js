...
export default function NotificationsPage() {
    ...
    useEffect(() => {
        ...
        const unsubscribe = notificationsApi.listenToNotifications(user, (data) => {
            // O'chirilgan xabarlarni filtrlaymiz (Kim o'chirgan bo'lsa o'shanga ko'rinmaydi)
    ...
    const openPreview = async (notif) => {
        // Tanlash rejimida xabarni ochmaslik kerak
    ...
    // 3. 🌟 TANLAB O'CHIRISH LOGIKASI (Yaxshilangan Checkbox)
    const toggleSelect = (id, e) => {
        if (e) e.stopPropagation(); // Card'ning klik hodisasini to'xtatish
    ...
    const handleJoinEvent = async (notif, action) => {
        try {
            // Adminning asl 'news' dokumentini qidiramiz (Sarlavha "Yangilik: " degan prefix bilan kelgan bo'lishi mumkin)
            const cleanTitle = notif.title.replace("Yangilik: ", "").trim();
            ...
            // Xabarning o'zida ham foydalanuvchi javob berganini belgilab qoyamiz
    ...
    // 5. 🌟 QAT'IY KATEGORIZATSIYA (Admin belgilagan Categoriyaga asoslanadi)
    const processedNotifications = useMemo(() => {
        ...
        return notifications.map(n => {
            ...
            // Xabarning "Haqiqiy" kategoriyasini aniqlash (DB dan qanday kelsa shunday yoki tahlil orqali)
            let smartType = n.type; 

            // DIQQAT: Agar xabar Admin tomondan e'lon orqali berilgan bo'lsa va unda aniq Category bo'lsa
            // Lekin hozirgi API'da n.type sifatida kelyapti deb faraz qilamiz.
            // Shuning uchun 'academic', 'events', 'sports', 'submissions', 'system' so'zlariga qat'iy o'giramiz:

            if (n.type === 'academic' || titleLower.includes('dars') || titleLower.includes('jadval') || msgLower.includes('xona')) {
                smartType = 'academic';
            } else if (n.type === 'sports' || titleLower.includes('sport') || titleLower.includes('turnir')) {
                smartType = 'sports';
            } else if (n.type === 'submissions' || titleLower.includes('vazifa') || titleLower.includes('javob') || titleLower.includes('baholan')) {
                smartType = 'submissions';
            } else if (n.type === 'events' || titleLower.includes('majlis') || titleLower.includes('tadbir')) {
                smartType = 'events';
            } else {
                smartType = 'system'; // Info yoki Warning yoki Umumiy E'lonlar
            }
    ...
    // UI da tanlangan Tab (Filter) ga qarab ma'lumotlarni saralash
            ...
            <div className="grid grid-cols-1 gap-3 md:gap-4">
                {loading ? (
                    ...
                    filteredNotifications.map((notification) => (
                        <Card
                            key={notification.id}
                            onDoubleClick={() => !isSelectMode && openPreview(notification)}
                            onClick={() => { if(isSelectMode) toggleSelect(notification.id) }} 
                            className={`p-0 overflow-hidden transition-all duration-300 border rounded-2xl md:rounded-[32px] cursor-pointer group select-none hover:shadow-lg relative
                        ...
                        >
                            <div className="p-4 md:p-6 flex flex-col sm:flex-row items-start sm:items-center space-y-4 sm:space-y-0 sm:space-x-4 md:space-x-6 relative">
                                ...
                                {/* 🌟 MUKAMMAL CHECKBOX: Qolgan joylardan to'liq ajratilgan */}
                                {isSelectMode && (
                                    <div className="absolute top-4 right-4 sm:static sm:top-auto sm:right-auto z-20" onClick={(e) => toggleSelect(notification.id, e)}>
                                        <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all cursor-pointer ${selectedIds.includes(notification.id) ? 'bg-indigo-500 border-indigo-500' : 'border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800'}`}>
                ...
                <div className="fixed inset-0 z-[700] flex items-center justify-center p-4 animate-in fade-in duration-200">
                    ...
                    <div className="relative w-full max-w-xl bg-white dark:bg-slate-900 rounded-[32px] md:rounded-[40px] shadow-2xl p-6 md:p-10 animate-in zoom-in-95 max-h-[85vh] flex flex-col border border-white/10">
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 mb-6">
                            ...
                            {/* Agar avval javob bergan bo'lsa */}
