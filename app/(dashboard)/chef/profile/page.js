...
import {
    User, Mail, Phone, BookOpen, ChefHat,
...
const CHEF_ROLES = ["Bosh Oshpaz", "Oshpaz", "Oshpaz Yordamchisi", "Qandolatchi", "Nonvoy", "Parhez taomlar mutaxassisi"];
            ...
            try {
                ...
                if (snap.exists()) {
                    ...
                    setForm({
                        ...
                        department: data.department || "", role: data.role || data.degree || "Bosh Oshpaz",
    ...
    const handleSave = async (e) => {
        ...
        try {
            await updateDoc(doc(db, "users", user.uid), {
                ...
                role: form.role, certificates: form.certificates
            });
    ...
    return (
        <div className="p-4 lg:p-10 w-full max-w-[1000px] mx-auto animate-in fade-in duration-500 pb-32">
            ...
            <div className="sticky top-4 z-[900] mb-8 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-100 dark:border-white/5 rounded-[24px] p-4 px-6 shadow-sm flex flex-col sm:flex-row justify-between items-center gap-4">
                <div>
                    <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tighter">Oshpaz Profili</h1>
                    <p className="text-slate-500 font-bold uppercase tracking-widest text-[9px] mt-0.5 flex items-center">
                        <ChefHat className="w-3 h-3 mr-1.5 text-indigo-500" /> Mutaxassislik va sertifikatlar
                    </p>
                </div>
            ...
            <form onSubmit={handleSave} className="space-y-6">
                ...
                <Card className="p-5 md:p-8 bg-white/80 dark:bg-slate-900/60 backdrop-blur-md border border-slate-100 dark:border-white/5 rounded-[32px] shadow-sm relative z-50">
                    <h3 className="text-sm font-black text-slate-900 dark:text-white mb-6 uppercase tracking-widest flex items-center"><ChefHat className="w-4 h-4 mr-2 text-emerald-500" /> Kasbiy Mutaxassislik</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-2 relative z-50">
                        <div className="space-y-1.5 relative z-50">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Mutaxassisligi (Lavozimi)</label>
                            <CustomSelect icon={Award} placeholder="Lavozimni tanlang" options={CHEF_ROLES} value={form.role} onChange={val => setForm({ ...form, role: val })} />
                        </div>
                        <div className="space-y-1.5 relative z-40">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Ish joyi bo'limi</label>
                            <div className="relative">
                                <Building2 className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                                <input type="text" value={form.department} onChange={e => setForm({ ...form, department: e.target.value })} className="w-full pl-11 pr-4 py-3.5 bg-slate-50 dark:bg-slate-800 rounded-xl text-sm font-bold dark:text-white outline-none focus:ring-2 focus:ring-emerald-500/20 border border-transparent focus:border-emerald-200" placeholder="Masalan: Markaziy Oshxona" />
                            </div>
                        </div>
                    </div>
                </Card>
                            ...
                            {form.certificates.map((cert, index) => (
                                <div key={index} className="flex flex-col md:flex-row gap-4 items-start md:items-center p-4 md:p-5 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-700">
                                    <div className="flex-1 w-full space-y-1.5">
                                        ...
                                        <input type="text" value={cert.title} onChange={e => handleCertChange(index, 'title', e.target.value)} className="w-full px-4 py-3.5 bg-white dark:bg-slate-900 rounded-xl text-xs font-bold outline-none border border-transparent focus:border-amber-200 shadow-sm" placeholder="Masalan: Sanitariya ruxsatnomasi" required />
                                    </div>
                                    <div className="flex-1 w-full space-y-1.5">
